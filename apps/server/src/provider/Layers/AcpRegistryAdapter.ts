...
import {
  ...
  applyAcpRegistryModelSelection,
  currentAcpRegistryModelIdFromSessionSetup,
  makeAcpRegistryRuntime,
  resolveAcpRegistryModelId,
...
const ACP_REGISTRY_RESUME_VERSION = 1 as const;
...
export interface AcpRegistryAdapterLiveOptions {
...
interface AcpRegistrySessionContext {
...
const resolveNotificationTurnId = (ctx: AcpRegistrySessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: AcpRegistrySessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, AcpRegistrySessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parseAcpRegistryResume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== ACP_REGISTRY_RESUME_VERSION) return undefined;
...
export function acpRegistryPromptSettlementBelongsToContext(input: {
...
export function makeAcpRegistryAdapter(
  acpRegistrySettings: AcpRegistrySettings,
  options?: AcpRegistryAdapterLiveOptions,
) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate ACP Registry runtime identifier.",
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process ACP Registry callback.",
              cause,
            }),
        ),
      );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = acpRegistryPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native ACP Registry notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const startSession: AcpRegistryAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const resumeSessionId = parseAcpRegistryResume(input.resumeCursor)?.sessionId;
          ...
          const requestedStartModelId = modelSelectionForInstance?.model
            ? resolveAcpRegistryModelId(modelSelectionForInstance.model)
            : undefined;
          const boundModelId = yield* applyAcpRegistryModelSelection({
            runtime: acp,
            currentModelId: currentAcpRegistryModelIdFromSessionSetup(started.sessionSetupResult),
            requestedModelId: requestedStartModelId,
            mapError: (cause) =>
              mapAcpToAdapterError(PROVIDER, input.threadId, "session/set_model", cause),
          });
          yield* applyAcpRegistryConfigOptionSelections({
            runtime: acp,
            selections: Array.isArray(modelSelectionForInstance?.options)
              ? modelSelectionForInstance.options
              : undefined,
            mapError: (cause) =>
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveAcpRegistryModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: ACP_REGISTRY_RESUME_VERSION,
          ...
          const ctx: AcpRegistrySessionContext = {
            threadId: input.threadId,
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process ACP Registry runtime notification.", { cause }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "ACP Registry session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveAcpRegistryModelId(turnModelSelection.model)
                : undefined;
              const currentModelId = yield* applyAcpRegistryModelSelection({
                runtime: ctx.acp,
              ...
              yield* applyAcpRegistryConfigOptionSelections({
                runtime: ctx.acp,
                selections: Array.isArray(turnModelSelection?.options)
              ...
              const displayModel = currentModelId
                ? resolveAcpRegistryModelId(currentModelId)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "ACP Registry prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "ACP Registry prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "ACP Registry session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "ACP Registry session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "ACP Registry prompt request failed.",
                }),
              );
    ...
    const interruptTurn: AcpRegistryAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const rollbackThread: AcpRegistryAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "ACP Registry sessions do not support provider-side rollback yet.",
        });
      });
