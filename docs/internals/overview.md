...
│ OpenCode, and configurable ACP Registry        │
└────────────────────────────────────────────────┘
```
...
Codex, Claude, Cursor, Grok, OpenCode, and ACP Registry. A driver declares its kind and config schema
and creates a scoped adapter; `ProviderInstanceRegistry` owns live instances and
`ProviderAdapterRegistry` resolves an instance to its adapter, so `ProviderService` routes session
and turn operations without knowing which agent is behind them. ACP Registry is instance-only (no
legacy `providers` blob) and speaks standards ACP only. See [providers.md](./providers.md).
